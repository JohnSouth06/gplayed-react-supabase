...
export const getPlatformInfo = (platform: string): { icon: string; bg: string; label: string } => {
  ...
  if (p.includes('android'))
    return { icon: 'android', bg: '#2ccb79', label: 'Android' };
  if (p.includes('wii u'))
...
export const getBaseStyles = (theme: typeof C, accentColor: string, accentColorDim: string) => {
