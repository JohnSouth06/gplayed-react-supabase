...
import React, { useEffect, useMemo, useState } from 'react'; //[cite: 11]
import {
  ActivityIndicator, Alert, Image, ScrollView, Switch,
  Text, TouchableOpacity, View, useColorScheme //[cite: 11]
} from 'react-native';
import { C, getProfileStyles } from './profile.styles';
...
export default function ProfileScreen() {
  ...
  [currentTheme, accentColor]); //[cite: 11, 15]
  ...
  const uploadAvatar = async (file: any) => {
    ...
    if (response.ok) { console.log("Image optimisée disponible ici :", data.url); }
  };
  ...
  const pickAndUploadAvatar = async () => {
    ...
    try {
      ...
      const formData = new FormData();
      // @ts-ignore
      formData.append('file', { uri: manipulatedImage.uri, name: fileName, type: 'image/jpeg' });
      ...
      setProgress(1);
    } catch (e: any) { Alert.alert("Erreur", e.message); } finally { setUploading(false); setProgress(0); }
  };
  ...
  if (loading) {
    return (<View style={styles.centered}><ActivityIndicator color={accentColor} size="large" /></View>);
  }
  ...
  const SectionTitle = ({ label, icon, danger }: any) => (
    <View style={sectionStyles.row}>
      <MaterialCommunityIcons name={icon} size={13} color={danger ? currentTheme.red : accentColor} />
      <Text style={[sectionStyles.label, danger && { color: currentTheme.red }]}>{label}</Text>
    </View>
  );

  const MenuRow = ({ icon, label, onPress, showChevron, danger }: any) => (
    <TouchableOpacity style={menuStyles.item} onPress={onPress} activeOpacity={0.75}>
      <View style={[menuStyles.iconWrap, { backgroundColor: danger ? currentTheme.redDim : `${accentColor}15` }]}>
        <MaterialCommunityIcons name={icon} size={17} color={danger ? currentTheme.red : accentColor} />
      </View>
      <Text style={[menuStyles.label, danger && { color: currentTheme.red }]}>{label}</Text>
      {showChevron && <MaterialCommunityIcons name="chevron-right" size={20} color={currentTheme.textMuted} />}
    </TouchableOpacity>
  );
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity onPress={pickAndUploadAvatar} disabled={uploading} activeOpacity={0.85}>
          <View style={styles.avatarRing}>
            <View style={styles.avatarWrapper}>
              {profile?.avatar_url ? (
                <Image source={{ uri: profile.avatar_url }} style={styles.avatarImage} key={profile.avatar_url} />
              ) : (<Text style={styles.avatarLetter}>{initials}</Text>)}
              {uploading && (<View style={styles.uploadOverlay}><ActivityIndicator color={accentColor} /></View>)}
            </View>
          </View>
          <View style={styles.editBadge}><MaterialCommunityIcons name="camera-outline" size={13} color={currentTheme.bg} /></View>
        </TouchableOpacity>

        {uploading && (<View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${progress * 100}%` }]} /></View>)}
        <View style={styles.usernamRow}><Text style={styles.username}>{profile?.username}</Text></View>
      ...
      <View style={[styles.card, styles.dangerCard]}><MenuRow icon="account-remove-outline" label="Supprimer mon compte" onPress={confirmDelete} danger /></View>

      <TouchableOpacity style={styles.logoutBtn} onPress={() => signOutUser()} activeOpacity={0.7}><MaterialCommunityIcons name="logout-variant" size={17} color={currentTheme.textMuted} /><Text style={styles.logoutLabel}>Se déconnecter</Text></TouchableOpacity>
    </ScrollView>
  );
}