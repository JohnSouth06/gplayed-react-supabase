...
const fetchGames = async () => {
    try {
      ...
      // On garde les changements d'états pour l'UI
      setLoading(false);
      setRefreshing(false);
    }
  };
  ...
  const addGameToCollection = async (game: any) => {
    try {
      ...
      // 1. CONVERSION DE LA DATE ICI
      let formattedReleaseDate = null;
      if (game.first_release_date) {
        const dateObj = new Date(game.first_release_date * 1000); // Conversion secondes -> millisecondes
        formattedReleaseDate = dateObj.toISOString().split('T')[0]; // Format YYYY-MM-DD
      }

      // 2. AJOUT DANS L'OBJET newGame
      const newGame = {
        user_id: user.id,
        igdb_id: game.id,
        title: game.name,
        ...
        status: 'A faire',
        playtime: 0,
        release_date: formattedReleaseDate // <-- NOUVELLE COLONNE AJOUTÉE ICI
      };
          ...
          if (viewMode === 'list') {
            return (
              <TouchableOpacity
                ...
                {...pressHandlers}
              >
                {/* Check circle en mode sélection */}
                {selectionMode && (
                ...
                {!selectionMode && (
                  <View style={[styles.listStatusBadge, { backgroundColor: `${statusColor}22`, borderColor: `${statusColor}55` }]}>
                    <MaterialCommunityIcons name={getStatusIcon(item.status) as any} size={13} color={statusColor} />
                    <Text style={[styles.listStatusText, { color: statusColor }]} numberOfLines={1}>{item.status}</Text>
                  </View>
                )}
              </TouchableOpacity>
            );
