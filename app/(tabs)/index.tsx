...
const C = {
  ...
  yellow: '#FFD34C',
  grey: '#c5dcd3',
  pink: '#FF6B9E', // Ajouté pour l'UI wishlist dans la recherche
};
...
export default function DashboardScreen() {
  ...
  const [myGames, setMyGames] = useState<any[]>([]); // Seulement la collection (sans wishlist)
  const [fullCollection, setFullCollection] = useState<any[]>([]); // Toute la base (utile pour la recherche)
  ...
  const fetchGames = async () => {
    try {
      ...
      // On sauvegarde tout pour la recherche (savoir si un jeu est en wishlist)
      setFullCollection(collectionData);
      
      // FILTRAGE : On exclut le statut 'wishlist' pour l'affichage du Dashboard
  ...
  const handleAddGame = async (gameData: any) => {
    try {
      ...
      // Vérifie si le jeu est dans la wishlist
      const wishlistEntry = fullCollection.find(
        (g) => g.game_id === gameData.id && g.platform === gameData.selectedPlatform && g.status === 'wishlist'
      );

      if (wishlistEntry) {
        // S'il est dans la wishlist, on le met à jour vers 'todo' (A faire)
          ...
          <View style={styles.modalSearchContent}>
            ...
            {searching ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  // Vérification globale (collection + wishlist)
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{ flex: 1, marginLeft: 14 }}>
                        ...
                        {isInWishlist && (
                          <Text style={{color: C.pink, fontSize: 10, marginTop: 2, fontWeight: '600'}}>
                            Dans votre wishlist
                          </Text>
                        )}
                      </View>
                      ...
                      <View style={[
                        ...
                        isInWishlist && { backgroundColor: C.pinkDim, borderColor: C.pink, borderWidth: 1 }
                      ]}>
                        <MaterialCommunityIcons 
                          name={isAlreadyAdded ? "check" : isInWishlist ? "library-plus" : "plus"} 
                          size={18} 
                          color={isAlreadyAdded ? C.textMuted : isInWishlist ? C.pink : C.bg} 
                        />
                      </View>
                    </TouchableOpacity>
                  );
